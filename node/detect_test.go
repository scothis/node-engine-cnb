...
func testDetect(t *testing.T, context spec.G, it spec.S) {
	...
	context("when the source code contains an .nvmrc file", func() {
		it.Before(func() {
			nvmrcParser.ParseCall.Returns.Version = "1.2.3"
		})
		...
		it("returns a plan that provides and requires that version of node", func() {
			...
			Expect(nvmrcParser.ParseCall.Receives.Path).To(Equal("/working-dir/.nvmrc"))
		})
	})

	context("when the source code contains a buildpack.yml file", func() {
		it.Before(func() {
			buildpackYMLParser.ParseCall.Returns.Version = "4.5.6"
		})
		...
		it("returns a plan that provides and requires that version of node", func() {
			...
			Expect(buildpackYMLParser.ParseCall.Receives.Path).To(Equal("/working-dir/buildpack.yml"))
		})
	})
	...
	context("failure cases", func() {
		context("when the nvmrc parser fails", func() {
			it.Before(func() {
				nvmrcParser.ParseCall.Returns.Err = errors.New("failed to parse .nvmrc")
			})
		...
		context("when the buildpack.yml parser fails", func() {
			it.Before(func() {
				buildpackYMLParser.ParseCall.Returns.Err = errors.New("failed to parse buildpack.yml")
			})
