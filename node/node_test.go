...
`
		...
		it.Focus("uses the default version when a version is not requested", func(){
			f.AddDependencyWithVersion(node.Dependency, "0.9", stubNodeFixture)
			f.AddDefaultDependency(node.Dependency, "0.9")
			f.AddBuildPlan(node.Dependency, buildplan.Dependency{})
		...
		when("optimize memory", func() {
			...
			it("sets NODE_OPTIONS to use 3/4ths available memory when buildpack.yml nodejs.optimize_memory is true", func() {
